...
from .models import Job, Application, UserProfile, JobAlert
from .forms import JobForm, ApplicationForm, UserProfileForm, JobAlertForm
...
def company_detail(request, company_id):
    ...
    jobs = Job.objects.filter(company=company).order_by('-date_posted')
...
def my_companies(request):
    ...
    for company in companies:
        company.jobs_count = company.job_set.count()
        company.active_jobs = company.job_set.count()  # You can add an 'active' field later
