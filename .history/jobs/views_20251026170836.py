...
@login_required
def network_connections(request, username=None):
    """Page showing all connections"""
    # Set profile_user - use provided username or current user
    if username:
    ...
    if tab == 'followers':
        connections = User.objects.filter(following__following=profile_user)
        title = 'Followers'
    else:
        connections = User.objects.filter(followers__follower=profile_user)
        title = 'Following'
    
    context = {
        'profile_user': profile_user,  # Make sure this is always set
        'connections': connections,
        'tab': tab,
        'title': title,
    }
